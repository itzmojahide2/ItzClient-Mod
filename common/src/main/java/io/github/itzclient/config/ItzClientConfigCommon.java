...
public class ItzClientConfig extends ItzClientConfigCommon {
    ...
    // --- Options from original client, now part of ItzClient ---
    ...
    // --- NEW UNIVERSAL HUD STYLING OPTIONS ---
    ...
    public ItzClientConfig() {
        // Add all categories to the main config
        config.add(general);
        config.add(rendering);
        config.add(hudSettings); // Add the new HUD settings category
        rendering.add(outlines);
        rendering.add(timeChanger);

        // Add options to their categories
        ...
        rendering.add(customSky, dynamicFOV, fullBright, removeVignette, lowFire, lowShield, hitColor, minimalViewBob, flatItems, noHurtCam);
        ...
        // Register keybinds with the correct category
