...
public class MemoryHud extends TextHudEntry implements DynamicallyPositionable {
    ...
    public void renderComponent(AxoRenderContext context, float delta) {
        ...
        if (showText.get()) {
            ...
            context.br$drawString(memoryLine, pos.x() + justification.get().getXOffset(context.br$getFont().br$getWidth(memoryLine), getWidth() - 4) + 2, pos.y() + (Math.round((float) getHeight() / 2) - 4) + yOffset, textColor.get().toInt(), shadow.get());
            if (showAllocated.get()) {
                context.br$drawString(allocationLine, pos.x() + justification.get().getXOffset(context.br$getFont().br$getWidth(allocationLine), getWidth() - 4) + 2, pos.y() + (Math.round((float) getHeight() / 2) - 4) + 4, textColor.get().toInt(), shadow.get());
            }
        }
    }
